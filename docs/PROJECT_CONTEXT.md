...
This document describes what the ECDAT codebase actually does today, as observed directly in the code on branch `ecdat-1` (last updated 2026-09-14, after (1) the backend-consolidation and AI-data-consistency fixes, (2) the AI Advisor UI/UX integration and dashboard visual-design pass, (3) the backend risk-scoring unification, (4) making `RiskContext` genuinely per-asset/evidence-derived, and (5) a dedicated full frontend UI/UX overhaul — see `CHANGELOG.md`). It is descriptive, not aspirational — see `TODO.md` for gaps and problems.
...
- **AI Migration Advisor** — Qwen3:14B (via Ollama) explains the ECDAT results for a selected asset in plain language, in a fixed 6-section format (RISK/MIGRATION/PQC/ACTIONS/IMPACT/SUMMARY, each now shown with a matching icon). Embedded directly in the asset detail view (not a separate page-bottom section) with an idle/loading/success/error+retry state machine and a factual data snapshot (risk, priority, complexity, blast radius, PQC recommendation, source impact) shown alongside the AI-generated text. See `AI_ADVISOR.md`.
- **Dashboard UI** — a hero overview (headline, readiness ring, top-level stat cards), a repository-analysis panel with a 6-stage pipeline visualization, bar charts (risk severity, migration type, source impact distribution via Recharts), a filterable/searchable card-based asset explorer (each asset card shows risk/priority/complexity/blast-radius/source-impact/PQC recommendation at a glance), and a per-asset detail panel (full-height overlay) presenting Identity → Risk/Priority → Impact Analysis → PQC Recommendation → Migration Actions → AI Advisor in that fixed order — all on one scrollable page with sidebar anchor-scroll navigation (no router). Restyled 2026-09-14 as a tokenized dark "security intelligence" design system, then substantially restructured and componentized later the same day in a dedicated frontend UI/UX overhaul — see `ARCHITECTURE.md` §8/§12.
...
- **Recharts** 3.10.1 for bar charts
...
- Plain hand-written CSS (`App.css`, rewritten 2026-09-14 as a single tokenized design-system stylesheet — CSS custom properties for colors/spacing/radius/shadows/motion; `index.css` is a 6-line reset) — no Tailwind/CSS-in-JS/component library
- `App.jsx` is still the single state/effects orchestrator (no router — `react-router` is not a dependency, in-page anchor scrolling), but is now a thin composition layer: nearly all rendering lives in `frontend/src/components/` (see the layout table below), extracted across two 2026-09-14 rounds — an initial presentational extraction (`Badge.jsx`, `StatCard.jsx`, `DistributionBar.jsx`, `States.jsx`, `AIAdvisorPanel.jsx`), then a full structural overhaul the same day adding `HeroOverview.jsx`, `PipelineStepper.jsx`, `RepositoryAnalysisPanel.jsx`, `AnalyticsPanel.jsx`, `Sidebar.jsx`, `AssetFilters.jsx`, `AssetExplorer.jsx`, `AssetDetailPanel.jsx`, and `detail/AssetIdentityHeader.jsx`/`RiskPriorityStrip.jsx`/`ImpactAnalysis.jsx`/`PQCRecommendation.jsx`/`MigrationActions.jsx`. No new dependency was added for either round — plain component extraction/creation, same React/Vite architecture throughout.
- No test setup (no Jest/Vitest/RTL config or test files under `frontend/`) — the 2026-09-14 UI work (both rounds) was instead verified with a throwaway, dependency-free Chrome DevTools Protocol driver script (Node's built-in `fetch`/`WebSocket` against a locally-launched headless Chrome), deleted after use each time; see `CHANGELOG.md` for what each round's script exercised.
...
frontend/
  ...
  AnalyticsPanel.jsx, Sidebar.jsx, AssetFilters.jsx, AssetExplorer.jsx,
  AssetDetailPanel.jsx
  src/components/detail/     AssetIdentityHeader.jsx, RiskPriorityStrip.jsx, ImpactAnalysis.jsx,
  PQCRecommendation.jsx, MigrationActions.jsx  (asset-detail sections, in
